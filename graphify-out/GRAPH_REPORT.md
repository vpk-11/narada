# Graph Report - narada  (2026-08-15)

## Corpus Check
- 41 files · ~69,672 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 278 nodes · 532 edges · 18 communities (14 shown, 4 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 46 edges (avg confidence: 0.58)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8c4be19a`
...
Cohesion: 0.23
Nodes (11): _cache_key(), _cache_path(), clear_cache(), get_cached(), core/cache.py  Simple disk-based cache for pipeline results. Keyed on a hash of, Generate a unique cache key from query + provider config.     Different provider, Return a cached PipelineResult if one exists for this query + config.     Return, Write a PipelineResult to disk cache.     Silently skips on any write error — ca (+3 more)
...
Cohesion: 0.22
Nodes (6): api/limiter.py  Shared rate limiter instance. Imported by main.py (to register o, Request, main.py  Narada — FastAPI entry point.  In production, serves the React frontend, Catch-all route that serves the React app for any non-API path.         React Ro, security_headers(), serve_frontend()

### Community 3 - "Web Scraping Pipeline"
Cohesion: 0.11
Nodes (44): analyze_query(), Analyze a user query and produce a structured research plan.      Args:, _build_settings_from_headers(), CacheClearResponse, delete_cache(), get_providers(), get_server_config(), ProviderInfo (+36 more)
...
## Knowledge Gaps
- **67 isolated node(s):** `name`, `private`, `version`, `type`, `packageManager` (+62 more)
...
- **Why does `BaseSearchProvider` connect `App Config and Security` to `Web Scraping Pipeline`?**
